    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(16,185,129,0.08),_transparent_28%),linear-gradient(180deg,_#f8fafc_0%,_#eef2ff_100%)]">
      ...
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-5 lg:px-6 lg:py-8">
        <AdminHero
          ...
          description={selectedEvent ? 'Run invite campaigns from a single WhatsApp Web device session, slow the queue down when needed, and monitor live progress without leaving the admin panel.' : 'Select an event first. Then connect WhatsApp Web, choose a safe sending profile, and start the invite queue.'}
        >
          {selectedEvent ? (
        ...
        <div className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
          <div className="space-y-6">
            <SurfaceCard eyebrow="Session" title="WhatsApp Web connection" description="This session controls WhatsApp Web from one logged-in device session. For Vercel deployments, run the local batch script because serverless functions cannot keep WhatsApp Web alive.">
                ...
                <div className="space-y-3">
                  ...
                  {isWaBlocked ? (
                    <div className="max-w-xl rounded-[24px] border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
                      ...
                      <p className="mt-1">{waState.reason || 'Run the WhatsApp Web batch script on a local machine or persistent server.'}</p>
                      <p className="mt-2 text-xs font-semibold uppercase tracking-[0.22em] text-amber-700">Use: npm run wa:send -- --event-id EVENT_ID</p>
                    </div>
                  ) : null}
            ...
            <SurfaceCard eyebrow="Queue" title="Start or stop the invite run" description="Only pending contacts are included. Confirmed contacts are excluded automatically.">
              <div className={`space-y-4 ${!isConnected ? 'pointer-events-none opacity-50' : ''}`}>
                <div className="rounded-[24px] border border-slate-200 bg-slate-50 px-4 py-4">
                  ...
                  <p className="mt-1 text-sm text-slate-500">{isWaBlocked ? 'Run the local WhatsApp Web script for Vercel deployments.' : 'Keep delays at 45 seconds or above for a safer delivery rhythm.'}</p>
                </div>
                <div className="grid gap-2 sm:flex">
                  {isRunning ? (
